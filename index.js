...
function askEvent(chatId) {
  bot.sendMessage(chatId, "🎉 Какое событие?", {
    reply_markup: {
      keyboard: [
        ["Корпоратив", "Свадьба"],
        ["Выпускной", "День рождения"],
        ["Обучение/Тимбилдинг", "Другое"],
      ],
...
function sendSummary(chatId) {
  ...
  const totalPrice =
    basePrice * guestFactor +
    (locationFactor > 1 ? basePrice * (locationFactor - 1) : locationFactor);
